...
trait Driver {
  ...
  lazy val decoders: PrefixForest =
    Decoder.merge(outputs.values.map(_.decoder), ignoreDuplicateDecoders) match {
      case Result.Err(e) =>
        throw new Exception(s"Failed to merge deciders due to $e")
      case Result.Res(v) => v
    }
  ...
  def startActors(names: Set[String], deviceId: String)(
    implicit system: ActorSystem
  ): SMap[ActorRef] =
    startActors(names.map(n => n -> BroadcastActor.props(n)).toMap, deviceId)

  def startActors(names: Set[String], deviceId: String, props: Props)(
        ...
        try {
          ...
          Res(
            RunningDriver(
              ...
              kill,
              deviceActor,
              companion.defaultConfigurations,
              extIns,
              extOuts
            )
          )
...
object Driver {
  ...
  implicit class ConnectorOps(val connector: Connector) extends AnyVal {
    def connect(device: String): Result[RunningDriver] =
      connector.f(device, connector)

    def wrapFlow(wrap: WrapFlow): Connector =
      connector.copy(
        wrap =
          if (connector.wrap == IdWrap) wrap else wrap.andThen(connector.wrap)
      )
  }

  case class Meta(inputs: Seq[String],
                  outputs: Seq[String],
                  name: String,
                  binPath: String)
  ...
  def source[T](producer: ActorRef): Source[T, _] =
    Source
      .actorRef(1, OverflowStrategy.dropHead)
  ...
  def sink[T](consumer: ActorRef): Sink[T, _] =
    Flow[T].to(ActorSink[T](consumer))
