...
import com.ilyak.pifarm.types.{ConnectState, SMap}
...
object ShapeConnections {
  ...
  case class ExternalConnections private (inputs: ExternalInputs,
                                          outputs: ExternalOutputs)

  object ExternalConnections {
    def apply(inputs: Seq[External.ExtIn[_]],
              outputs: Seq[External.ExtOut[_]]): ExternalConnections =
  ...
  object AutomatonConnections {
    ...
    def apply(inputs: Seq[Connection.In[_]],
              outputs: Seq[Connection.Out[_]],
              shape: ConnectState): AutomatonConnections =
  ...
  implicit val inMap: ShapesConnections[Connection.In] =
    new ShapesConnections[Connection.In] {
      override def get[S <: ShapeConnections](s: S): SMap[Connection.In[_]] =
        s.inputs
    }

  implicit val outMap: ShapesConnections[Connection.Out] =
    new ShapesConnections[Connection.Out] {
      override def get[S <: ShapeConnections](s: S): SMap[Connection.Out[_]] =
        s.outputs
    }
}
