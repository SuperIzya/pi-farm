...
object Result {
  ...
  def combineB[T1, T2, T3](x: Result[T1], y: Result[T2])
                          (f: (T1, T2) => Result[T3]): Result[T3] =
    combine(x, y)(f).flatMap { i => i }

  def combine[T1, T2, T3](x: Result[T1], y: Result[T2])
                         (f: (T1, T2) => T3): Result[T3] =
    (x, y) match {
      case (Res(a), Res(b)) => Res(f(a, b))
      case (Err(l1), Err(l2)) => Err(
        s"""
           |$l1
           |$l2
          """.stripMargin)
  ...
  def foldResults[S, E](append: (S, E) => S): (Result[S], Result[E]) => Result[S] =
    combine(_, _)(append(_, _))

  def foldResultsT[T](append: (T, T) => T): (Result[T], Result[T]) => Result[T] =
  ...
  def fold[T, R](l: TraversableOnce[Result[T]])(init: R, combine: (R, T) => R): Result[R] =
