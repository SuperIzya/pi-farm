...
object Decoder {
  ...
  object Trie {
    ...
    private val alphabet: String = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 -_"
    ...
    implicit class TrieOps(val forest: PrefixForest) extends AnyVal {
      def getPrefix(key: String): Result[Decoder[_]] =
        getIndex(key)
          .flatMap { _ =>
            forest.get(key(0)).map(Result.Res(_))
              .getOrElse(Result.Err(s"No decoder for key $key"))
          }
          .flatMap { t =>
            if (t.key == key)
              t.value.map(Result.Res(_))
        ...
        getIndex(key) match {
          ...
          case Result.Res(_) =>
            forest.get(key(0)) match {
              case None =>
                Result.Res(forest ++ Map(key(0) -> Trie(key, Some(decoder), emptyForest)))
              ...
              case Some(trie) =>
                ...
                Result.Res {
                  forest ++ Map(sharedKey(0) -> Trie(sharedKey, None, Map(
  ...
  class DecoderShape(forest: PrefixForest) extends GraphStage[FlowShape[String, List[Any]]] {
        ...
        setHandler(in, new InHandler {
          override def onPush(): Unit = {
            ...
            forest.getPrefix(split(0)) match {
              ...
              case Result.Err(e) => log.error(s"Failed to decode message '$msg' due to '$e'")
            }
            pull(in)
          }
        })
