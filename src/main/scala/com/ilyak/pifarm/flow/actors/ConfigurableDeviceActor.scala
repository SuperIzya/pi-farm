...
import com.ilyak.pifarm.flow.actors.ConfigurationsActor.{
  Configurations,
  GetConfigurations
}
...
class ConfigurableDeviceActor(socketActors: SocketActors,
                              ...
                              driver: ActorRef)(implicit db: Database,
                                                profile: JdbcProfile,
  ...
  def loadConfigs(device: String): Future[Unit] = {
    ...
    val query = for {
      ...
      configs <- ConfigurationAssignmentTable
      if configs.device === driver.device
      configNames <- ConfigurationsTable
      if configNames.name === configs.configuration
    ...
    db.run {
      ...
      .map { names =>
        self ! LoadConfigs(
          device,
          names.toSet ++ drivers(device).defaultConfigurations.map(_.name)
        )
      }
  }

  def getAllConfigs: SMap[Set[String]] =
    configsPerDevice.mapValues(_.collect { case (k, _) => k })
  ...
  socketActors.actor ! RegisterReceiver(self, {
    case m @ StopConfig(device, driver, _) if thisDriver(device, driver) =>
      self ! m
    case m @ AssignConfig(device, driver, _) if thisDriver(device, driver) =>
      self ! m
  })
  ...
  override def receive: Receive = {
    ...
    case DriversList(d) =>
      configurations ++= d
        .flatMap(_.defaultConfigurations)
        .map(c => c.name -> c)
        .toMap
    ...
    case LoadConfigs(device, configs) =>
      ...
      val loc: String => PluginLocator = s =>
        loader.forRun(RunInfo(device, driver, s, conn.deviceActor))
      val loaded = configs
        .collect {
          ...
          case s if !configurations.contains(s) =>
            s -> Result.Err(s"configuration '$s' not found")
        }
        .collect {
      ...
      log.debug(
        s"On device $device with driver $driver loaded configurations: $configs"
      )
    ...
    case StopConfig(device, _, configs) =>
      ...
      configsPerDevice += device -> oldSet.filter {
        case (k, _) => configs.contains(k)
      }
      configActor ! AllConfigs(getAllConfigs)
    ...
    case AssignConfig(device, driver, configs) =>
      ...
      if (oldSet.collect { case (k, _) => k } != configs) {
        ...
        log.debug(
          s"Assigning to device $device with driver $driver configurations $configs"
        )
        ...
        val ins = for {
          ...
          if dev.device === device && dev.driver === driver
        ...
        val future = db.run {
          DBIO.seq(
            ...
            ins.result
              .map(
                _.map(
                  f => Tables.ConfigurationAssignment(Some(f._2), Some(f._1))
                )
              )
              .map(Tables.ConfigurationAssignmentTable.forceInsertAll(_))
          )
        } map { _ =>
          self ! LoadConfigs(
            device,
            configs ++ drivers(device).defaultConfigurations.map(_.name)
          )
        }
...
object ConfigurableDeviceActor {
  def props(socketActors: SocketActors,
            driver: ActorRef,
            configsActor: ActorRef)(implicit db: Database,
                                    profile: JdbcProfile,
                                    materializer: Materializer,
  ...
  case class StopConfig(device: String,
                        driver: String,
                        configurations: Set[String])
      extends JsContract
