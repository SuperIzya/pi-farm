...
import com.ilyak.pifarm.driver.control._
...
class HumidityMotorDriver
  ...
  override def connector(
    deviceProps: Props
  )(implicit s: ActorSystem, mat: ActorMaterializer): Driver.Connector =
    super
      .connector(deviceProps)
      .wrapFlow(_.via(RateGuard.flow[String](10, 1 minute)))
}

object HumidityMotorDriver
    extends DriverCompanion[HumidityMotorDriver]
    with ArduinoControl {
