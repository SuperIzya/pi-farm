...
const HandleList = ({ endpoints, direction }: HandleListProps) => (
  <div className={styles.handles}>
    {endpoints.filter(endpoint => endpoint.direction === direction).map((v, idx, {length}) => (
      <Tooltip 
        key={`${direction}-${idx}`}
        title={
...
export const ProcessingUnitNode = addDispatchProcessor(withStartDrag(({ 
  ...
  <div className={styles.node}>
    <div className={styles.dragHandle}
      onPointerDown={evt => onDragStart(evt, { type: 'processingUnit', itemKey }, addProcessorNode(id, itemKey, endpoints))}
    >
      <IconButton>
        <OpenWithIcon />
      </IconButton>
    </div>
    <GenericButton className={styles.delete} onClick={() => onDelete(id)} Icon={() => <DeleteForeverIcon />} />
...
export const ControllerNode = addDispatchController(withStartDrag(({ data: { id, itemKey, endpoints }, onDelete, addControllerNode, onDragStart }: NodeProps<ControllerNode> & WithActions<ControllerId> & WithStartDrag) => (
  <div className={styles.node}>
    <div className={styles.dragHandle}
      onPointerDown={evt => onDragStart(evt, { type: 'controller', itemKey }, addControllerNode(id, itemKey, endpoints))}
    >
      <IconButton>
        <OpenWithIcon />
      </IconButton>
