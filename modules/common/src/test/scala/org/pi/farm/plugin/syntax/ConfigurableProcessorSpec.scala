...
object ConfigurableProcessorSpec extends ZIOSpecDefault {
  ...
  def runPipeline(
    cp: ConfigurableProcessor,
    config: Configuration,
    messages: Chunk[Inbound]
  )(using zio.Trace): ZIO[Any, Throwable, Chunk[Outbound]] =
    for {
      pipeline <- cp.configure(config)
      result   <- ZStream
        .fromChunk(messages)
        .via(pipeline)
        .runCollect
  ...
  def spec = suite("ConfigurableProcessor")(
    ...
    suite("ConsumerProcessor")(
      test("processes matching DataPacket and emits nothing") {
        ...
        for {
          ...
          pp = new Pp(received)
          out <- runPipeline(pp.work, config, Chunk(mkDataPacket(cid1, pid1, 42)))
          got <- received.get
      ...
      test("ignores DataPacket not matching any inlet") {
        ...
        for {
          ...
          pp = new Pp(called)
          out <- runPipeline(pp.work, config, Chunk(mkDataPacket(cid2, pid2, 99)))
          c   <- called.get
    ...
    suite("InOutPProcessor")(
      test("transforms input to Command output") {
        ...
        runPipeline(Pp.work, config, Chunk(mkDataPacket(cid1, pid1, 21))).map { out =>
    ...
    suite("Multiple inlets and outlets")(
      test("processes multiple inlets and outlets") {
        ...
        val config = mkConfig(
          ...
          outbound = Chunk(Address(cid2, pid3, "x"), Address(cid2, pid2, "y"))
        )
        runPipeline(Pp.work, config, Chunk(mkDataPacket(cid1, pid1, 5), mkDataPacket(cid1, pid2, "hello"))).map { out =>
          val cmdX = out.filter(_.controllerId == cid2)
          val cmdY = out.find(_.controllerId == cid2)
          assertTrue(
            /* cmdX.flatMap(_.dataPoints).size == 1,
            cmdX.flatMap(_.dataPoints).head.peripheryId == pid3,
            cmdX.flatMap(_.dataPoints).head.data.as[Int].toOption.get == 10,
            cmdY.controllerId == cid2,
            cmdY.dataPoints.size == 1,
            cmdY.dataPoints.head.peripheryId == pid2,
            cmdY.dataPoints.head.data.as[String].toOption.get == "olleh" */
          )
        }
      }
    )
